...
class HAttention1D(nn.Module):
    ...
    def forward(self, x, mask=None):
        b, n, h, device, bsz, eps = * \
            x.shape[:2], self.heads, x.device, self.block_size, self.eps
        ...
        if exists(self.pos_emb):
            freqs = self.pos_emb(torch.arange(
                pad_to_len, device=device), cache_key=pad_to_len)
        ...
        # duplicate the finest resolution an extra time, for the base diagonal
        qkvs = [qkvs[0], *qkvs]
        ...
        def to_blocks(t): return einops.rearrange(
            t, 'b (n z) ... -> b n z ...', z=bsz)
        ...
        for ind, (q, k, v, mask) in enumerate(reversed(qkvs)):
            ...
            if exists(mask):
                ...
                S_mask = einops.rearrange(
                    q_mask, '... n -> ... n ()') * einops.rearrange(k_mask, '... n -> ... () n')
...
class CausalHAttention1D(nn.Module):
    def __init__(
        ...
        *,
        ...
        for ind in range(num_levels):
            ...
            expanded_mask_seq = einops.repeat(
                seq, 'n -> (n r)', r=(2 ** (ind + 1)))
    ...
    def forward(self, x, **kwargs):
        ...
        def calculate_Y_and_A(q, k, v, mask_right_off_diagonals=False, causal_mask_diagonal=False):
            ...
            if causal_mask_diagonal:
                causal_mask = torch.ones(
                    *S.shape[-2:], device=S.device).triu(1).bool()
                mask_value = -torch.finfo(S.dtype).max
            ...
            if mask_right_off_diagonals:
                y, A = map(lambda t: einops.rearrange(
                    t, 'b n ... -> b n () ...'), (y, A))
        ...
        for ind, (q, k, v) in enumerate(reversed(qkvs)):
            ...
            Y_level = calculate_Y_and_A(
                q, k, v, mask_right_off_diagonals=not is_last, causal_mask_diagonal=is_last)
            Ys.append(Y_level)
        ...
        for Y_level, A_level in Ys:
            Y_level, A_level = map(lambda t: einops.rearrange(
                t, '... -> () ...'), (Y_level, A_level))
...
class HTransformer1D(nn.Module):
    def __init__(
        ...
        *,
        ...
        dim_head=64,
        ff_mult=4,
        # this is the Nr in the paper - Nb = (max_seq_len / tokens_per_block)
        block_size=128,
        pos_emb=None,
        reversible=False,
        shift_tokens=False
        ...
        super().__init__()
        assert (max_seq_len %
                block_size) == 0, 'maximum sequence length must be divisible by the block size'
        num_blocks = max_seq_len // block_size
        assert log2(max_seq_len // block_size).is_integer(
        ...
        for ind in range(depth):
            attn = attn_class(dim, dim_head=dim_head, heads=heads,
                              block_size=block_size, pos_emb=self.pos_emb, **attn_kwargs)
            ff = FeedForward(dim, mult=ff_mult)

            if shift_tokens:
                attn, ff = map(lambda t: PreShiftTokens(
                    shift_token_ranges, t), (attn, ff))
...
class AutoregressiveWrapper(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        device = kwargs.get("device", torch.device(
