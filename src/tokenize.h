...
struct CompressedAlphabet {
    ...
    constexpr CompressedAlphabet(const CompressedAlphabet &) = default;
    constexpr CompressedAlphabet(CompressedAlphabet &&) = default;
    constexpr CompressedAlphabet(const char *name, const char *s, bool padding=false): name(name), setstr(s), nc(ncommas(s)), padding(padding), lut(make_lut(s, nc, padding)) {
    }
};


// Protein Alphabets
const CompressedAlphabet AMINO20("Standard20", "A,C,D,E,F,G,H,I,K,L,M,N,P,Q,R,S,T,V,W,Y");

const CompressedAlphabet SEB14("SE-B(14)", "A,C,D,EQ,FY,G,H,IV,KR,LM,N,P,ST,W");

const CompressedAlphabet SEB10("SE-B(10)", "AST,C,DN,EQ,FY,G,HW,ILMV,KR,P");
const CompressedAlphabet SEV10("SE-V(10)", "AST,C,DEN,FY,G,H,ILMV,KQR,P,W");
const CompressedAlphabet SOLISD("Solis-D", "AM,C,DNS,EKQR,F,GP,HT,IV,LY,W");
const CompressedAlphabet SOLISG("Solis-G", "AEFIKLMQRVW,C,D,G,H,N,P,S,T,Y");
const CompressedAlphabet MURPHY("Murphy", "A,C,DENQ,FWY,G,H,ILMV,KR,P,ST");
const CompressedAlphabet LIA10("Li-A(10)", "AC,DE,FWY,G,HN,IV,KQR,LM,P,ST");
const CompressedAlphabet LIB10("Li-B(10)", "AST,C,DEQ,FWY,G,HN,IV,KR,LM,P");

const CompressedAlphabet SEB8("SE-B(8)","AST,C,DHN,EKQR,FWY,G,ILMV,P");
const CompressedAlphabet SEB6("SE-B(6)","AST,CP,DHNEKQR,FWY,G,ILMV");

const CompressedAlphabet DAYHOFF("Dayhoff","AGPST,C,DENQ,FWY,HKR,ILMV");

// DNA alphabets

const CompressedAlphabet DNA4("DNA4", "A,C,G,T");
const CompressedAlphabet DNA5("DNA5", "A,C,G,T,NMRWSYKVHDB");

const CompressedAlphabet DNA2KETAMINE("DNA2", "ACM,KGT"); // Amino/Ketones
const CompressedAlphabet DNA2PYRPUR("DNA2", "AGR,YCT"); // Purines/Pyrimidines
...
static const std::map<std::string, const CompressedAlphabet *> CAMAP = {
    {"AMINO20", &AMINO20},
    {"AMINO", &AMINO20},
    {"PROTEIN", &AMINO20},
    {"SEB8", &SEB8},
...
struct Tokenizer {
    ...
    Tokenizer(const CompressedAlphabet &ca, bool eos=false, bool bos=false, bool zero_onehot_pad=false): ca_(&ca), include_eos_(eos), include_bos_(bos), zero_onehot_pad_(zero_onehot_pad) {
    }
